            ...
            <div class="container-fluid px-3 px-lg-4 py-4">
                <div class="page-heading">
                    ...
                    <div class="heading-actions"><a class="btn btn-outline-secondary btn-sm"
                            href="{{ route('admin.users') }}"><i class="bi bi-arrow-left" aria-hidden="true"></i> Back
                            to Users</a></div>
                </div>

                <section class="row g-3">
                    <div class="col-12 col-xl-8">
                        <form action="{{ route('admin.user-details.update',$user->id) }}" method="POST" class="panel needs-validation"
                            novalidate>
                            @csrf
                            @method('PUT')
                            ...
                            <div class="row g-3">
                                <div class="col-md-6"><label class="form-label" for="firstName">Full
                                        name</label><input class="form-control" placeholder="Full Name" id="firstName"  value="{{old('name',$user->name)}}"
                                ...
                                <div class="col-md-6"><label class="form-label" for="email">Email</label><input
                                        class="form-control" id="email" value="{{old('email',$user->email)}}" placeholder="Email Address" name="email"
                                        type="email" required>
                                    @error('email')
                                        <span class="text-danger">{{ $message }}</span>
                                    @enderror
                                ...
                                <div class="col-md-6"><label class="form-label" for="phone">Phone</label><input
                                        class="form-control" id="phone" placeholder="Phone" name="phone"  value="{{old('phone',$user->phone)}}"
                                        type="tel" required>
                                    @error('phone')
                                        <span class="text-danger">{{ $message }}</span>
                                    @enderror
                                ...
                                <div class="col-md-6"><label class="form-label" for="email">Confirm
                                        ...
                                        placeholder="Re type Password" name="password_confirmation" type="text">
                                    @error('password')
                                        <span class="text-danger">{{ $message }}</span>
                                    @enderror
                                </div>
                            ...
                            <div class="d-flex flex-wrap justify-content-end gap-2 mt-4"><a
                                    ...
                                    class="btn btn-primary" type="submit"><i class="bi bi-person-check"
                                        aria-hidden="true"></i> Update User</button></div>
                        </form>
                    </div>

    ...
    <section class="auth-card">
        ...
        <form action="{{route('authenticate')}}" method="POST" class="needs-validation" novalidate>
            ...
            <div class="mb-3"><label class="form-label" for="loginEmail">Email address</label><input
                    class="form-control" id="loginEmail" type="email" name="email" placeholder="Email Address" required>
                @error('email')
            ...
            <div class="mb-3">
                <div class="d-flex justify-content-between"><label class="form-label"
                        ...
                        href="{{ route('admin.forget-password') }}">Forgot?</a></div><input class="form-control"
                    name="password" id="loginPassword" placeholder="Password" type="password" minlength="6" required>
                @error('password')
                <span class="text-danger">{{$message}}</span>
                @enderror
            </div>
